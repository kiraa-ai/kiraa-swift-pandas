...
/// One-shot CSV transformation pipeline — the original `swiftpandas` behavior.
///
/// This subcommand preserves the legacy command line exactly:
///
///   swiftpandas -i in.csv -o out.csv -c "filter(...) | sort(...)"
///   swiftpandas -i in.csv -f transforms.json
///   swiftpandas --gui
///
/// It is registered as the `defaultSubcommand` on the root, so the flags
/// continue to work without typing `run` explicitly. Use this when you do not
/// want a long-lived daemon and have a single transform to apply to a single
/// CSV file.
struct Run: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "run",
        abstract: "Run a one-shot CSV transformation (no server required).",
        discussion: """
        Transform CSV data using inline DSL chains (-c) or JSON transform files (-f).

        Inline DSL example:
          swiftpandas run -i data.csv -o out.csv -c "filter(revenue > 10000) | sort(revenue, desc)"

        JSON file example:
          swiftpandas run -i data.csv -o out.csv -f transforms.json
    ...
    private func printOperationHelp() {
        ...
        print("  swiftpandas run -i data.csv -c \"filter(revenue > 10000) | sort(revenue, desc) | head(10)\"\n")
