#!/bin/bash
# 11 — Large CSV (100K rows): the most apples-to-apples benchmark
set -euo pipefail
...
OUT_PD="$DATA_DIR/summary_pandas.csv"
OUT_SP="$DATA_DIR/summary_swiftpandas.csv"
mkdir -p "$DATA_DIR"

# Generate the input CSV on first run (idempotent thereafter).
if [ ! -f "$INPUT" ]; then
    printf "${DIM}First run — generating 1,000,000-row transaction dataset…${RESET}\n"
    $PY -c "
...
for _ in range(1_000_000):
    ...
    w.writerow([random.choice(regions), random.choice(products),
                random.choice(statuses), revenue, cost, units])
" > "$INPUT"
fi
ROWS=$(($(wc -l < "$INPUT") - 1))
SIZE=$(du -h "$INPUT" | cut -f1)

script_banner "11 — Large CSV: 100K-row Group + Sum" "The most apples-to-apples timing in this directory"
script_meta "Input:" "$INPUT"
script_meta "Size:"  "$ROWS rows × 6 cols ($SIZE)"
script_meta "Both:"  "Read CSV → filter → groupby(region, product) → sums → sort → write CSV"

demo_intro <<EOF
What it does:
  A realistic ETL on a synthetic 100,000-row transaction dataset. The
  pandas side runs as a one-shot Python script (cold start + import +
  pipeline). The swiftpandas side runs 'load → pipe → save → drop'
  through the daemon.

Expected outcome: swiftpandas ~1.5-2.5× faster (the CLOSEST of all demos).

Why:
  • At 100K rows the actual CSV parse cost finally becomes visible:
    ~70 ms in both implementations. The 650 ms pandas import tax is
    no longer the dominant factor; it's still significant but not
    overwhelming.
  • Aggregation throughput is comparable: numpy reductions vs Accelerate
    vDSP, both ~10 ms on this dataset.
  • swiftpandas still wins on cold-start. But the more interesting
    number is **subsequent transforms on the same loaded DataFrame**:
    once 'tx' is resident, each new 'pipe' takes ~15 ms — including
    re-parses, pandas would need 650+70 = ~720 ms each time.
  • Run this script ten times sequentially via 'for i in {1..10}; do …':
    pandas takes ~10 × (parse+pipeline) ≈ 2.3 s; swiftpandas with a
    persistent daemon takes ~1× load + 10× ~15 ms ≈ 150 ms.
EOF
...
df = pd.read_csv("transactions_100k.csv")
...
result.to_csv("summary_pandas.csv", index=False)
EOF
    output_label
    time_start
    $PY <<PYEOF
...
for c in ("revenue", "cost", "profit"):
    result[c] = result[c].round(2)
result.to_csv("$OUT_PD", index=False)
print(f"    pandas wrote {len(result)} rows → $OUT_PD")
PYEOF
...
swiftpandas load transactions_100k.csv --name tx
swiftpandas pipe --from tx --name summary \\
    -c 'filter(status == "completed") |
        groupby(region, product) |
        agg(sum:revenue, sum:cost, sum:units) |
        derive(profit = revenue - cost) |
        sort(revenue, desc) | round(revenue, 2) |
        round(cost, 2)      | round(profit, 2)'
swiftpandas save summary summary_swiftpandas.csv
EOF
output_label
time_start
"$SWIFTPANDAS" load "$INPUT" --name tx >/dev/null
"$SWIFTPANDAS" pipe --from tx --name summary -c "
    ...
    round(profit, 2)
" >/dev/null
"$SWIFTPANDAS" save summary "$OUT_SP" >/dev/null
echo "    swiftpandas wrote $(($(wc -l < "$OUT_SP") - 1)) rows → $OUT_SP"
"$SWIFTPANDAS" drop tx      >/dev/null
"$SWIFTPANDAS" drop summary >/dev/null
time_marker_and_save "$(time_end_ms)" SP_T
rm -f "$OUT_PD" "$OUT_SP"

summary_block "$PANDAS_T" "$SP_T"
printf "${DIM}  Note: swiftpandas timing includes the load (CSV parse) — same as pd.read_csv.${RESET}\n"
printf "${DIM}  If you keep the daemon and 'tx' loaded across many transforms, each follow-up${RESET}\n"
printf "${DIM}  pipe is a sub-15 ms wire round trip (no parse cost).${RESET}\n"
script_footer
