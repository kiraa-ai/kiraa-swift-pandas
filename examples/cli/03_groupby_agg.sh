...
script_banner "03 — GroupBy + Aggregate" "Sum revenue, mean margin, count transactions per region"
script_meta "Input:"    "$DEMO_CSV"
script_meta "Pipeline:" 'groupby(region) | agg(sum:revenue, mean:margin, count:transactions)'

demo_intro <<EOF
What it does:
  Group rows by region; compute three aggregates per group (sum, mean,
  count). Produces a one-row-per-region summary.

Expected outcome: swiftpandas ~3-5× faster than pandas.

Why:
  • Same import tax as before — pandas pays ~650 ms on startup.
  • The aggregation itself is fast in both (vectorised reductions). On
    this 8-row dataset the actual compute is well under a millisecond.
  • The gap narrows compared to demo 01 because swiftpandas's wire round
    trip for an aggregation reply with three named columns is a few
    milliseconds heavier than a simple filter reply. Still: 'import
    pandas' dominates everything.
EOF
...
swiftpandas pipe --from sales --name r03 \\
    -c 'groupby(region) | agg(sum:revenue, mean:margin, count:transactions)'
swiftpandas show r03
swiftpandas server stop                        # (stopped on script exit)
EOF
output_label
time_start
"$SWIFTPANDAS" pipe --from sales --name r03 \
  -c 'groupby(region) | agg(sum:revenue, mean:margin, count:transactions)' >/dev/null
"$SWIFTPANDAS" show r03 | sed 's/^/    /'
