#!/bin/bash
# 09 — Write output: full pipeline producing a CSV file
set -euo pipefail
...
OUT_PD="/tmp/swiftpandas_demo_pandas.csv"
OUT_SP="/tmp/swiftpandas_demo_swiftpandas.csv"

script_banner "09 — Write Output" "Full pipeline → CSV file on disk"
script_meta "Input:"  "$DEMO_CSV"
script_meta "Output:" "/tmp/swiftpandas_demo_*.csv (auto-deleted after compare)"

demo_intro <<EOF
What it does:
  End-to-end ETL: filter active orders, group by (region, quarter), agg
  revenue + margin, sort, rename, round, write to CSV.

Expected outcome: swiftpandas ~3-5× faster than pandas.

Why:
  • Same import tax for pandas.
  • Both write the same shape of CSV (5 rows × 4 cols).
  • swiftpandas's CSV writer is a direct contiguous-buffer dump — no
    Python interpreter, no pandas index column to suppress, no float
    formatter to negotiate.
  • The wire payload here is only the result row count + bytes; the
    actual CSV stays server-side until 'save' writes it directly.
EOF
...
swiftpandas pipe --from sales --name r09 \\
  -c 'filter(status == "active") | groupby(region, quarter) |
      agg(sum:revenue, mean:margin) | sort(revenue, desc) |
      rename(revenue -> total_revenue) | round(margin, 2)'
swiftpandas save r09 /tmp/out.csv
swiftpandas server stop                        # (stopped on script exit)
EOF
output_label
time_start
"$SWIFTPANDAS" pipe --from sales --name r09 \
  -c 'filter(status == "active") | groupby(region, quarter) | agg(sum:revenue, mean:margin) | sort(revenue, desc) | rename(revenue -> total_revenue) | round(margin, 2)' >/dev/null
"$SWIFTPANDAS" save r09 "$OUT_SP" >/dev/null
cat "$OUT_SP" | sed 's/^/    /'
