...
script_banner "04 — Derive Computed Column" "Add profit = revenue - cost, then filter and sort"
script_meta "Input:"    "$DEMO_CSV"
script_meta "Pipeline:" 'derive(profit = revenue - cost) | filter(profit > 10000) | select(...) | sort(profit, desc)'

demo_intro <<EOF
What it does:
  Compute a new 'profit' column from two existing columns, filter by it,
  project a subset of columns, sort. Four chained ops.

Expected outcome: swiftpandas ~5-10× faster than pandas.

Why:
  • Same import tax as before.
  • Column arithmetic (revenue - cost) runs as SIMD-vectorised vDSP code
    in swiftpandas via Apple Accelerate. In pandas it goes through NumPy,
    which is also vectorised but pays interpreter overhead per call.
  • The derive + filter + select + sort chain happens server-side in one
    request — the client never sees the intermediate columns.
EOF
...
result = (df[df["profit"] > 10000]
            [["region", "sku", "revenue", "cost", "profit"]]
...
swiftpandas pipe --from sales --name r04 \\
    -c 'derive(profit = revenue - cost) | filter(profit > 10000) |
        select(region, sku, revenue, cost, profit) | sort(profit, desc)'
swiftpandas show r04
swiftpandas server stop                        # (stopped on script exit)
EOF
output_label
time_start
"$SWIFTPANDAS" pipe --from sales --name r04 \
  -c 'derive(profit = revenue - cost) | filter(profit > 10000) | select(region, sku, revenue, cost, profit) | sort(profit, desc)' >/dev/null
"$SWIFTPANDAS" show r04 | sed 's/^/    /'
