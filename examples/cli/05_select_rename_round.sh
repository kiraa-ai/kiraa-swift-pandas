...
script_banner "05 — Select / Rename / Round" "Project a subset of columns, rename one, round margin"
script_meta "Input:"    "$DEMO_CSV"
script_meta "Pipeline:" 'select(region, quarter, revenue, margin) | rename(revenue -> sales_amount) | round(margin, 1)'

demo_intro <<EOF
What it does:
  Trivial schema-shape operations: pick 4 columns, rename one, round
  another. No filtering, no aggregation, no compute.

Expected outcome: swiftpandas wins by the LARGEST margin in this directory
                  (likely 8-10×).

Why:
  • The actual work — copy 4 column references, rename a key, round 8
    floats — finishes in microseconds in BOTH implementations.
  • So the timing comparison effectively measures **startup overhead and
    nothing else.**
  • Python: ~50 ms interpreter + ~600 ms 'import pandas' = ~650 ms.
  • swiftpandas: socket dial + JSON encode + reply decode = ~5 ms.
  • This demo isolates the cost of "warming up a heavy library" vs.
    "calling a hot daemon".
EOF
...
df = pd.read_csv("sales.csv")
result = (df[["region", "quarter", "revenue", "margin"]]
            .rename(columns={"revenue": "sales_amount"})
            .assign(margin=lambda d: d["margin"].round(1)))
...
swiftpandas pipe --from sales --name r05 \\
    -c 'select(region, quarter, revenue, margin) |
        rename(revenue -> sales_amount) |
        round(margin, 1)'
swiftpandas show r05
swiftpandas server stop                        # (stopped on script exit)
EOF
output_label
time_start
"$SWIFTPANDAS" pipe --from sales --name r05 \
  -c 'select(region, quarter, revenue, margin) | rename(revenue -> sales_amount) | round(margin, 1)' >/dev/null
"$SWIFTPANDAS" show r05 | sed 's/^/    /'
