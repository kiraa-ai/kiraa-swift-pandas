#!/bin/bash
# 10 — Error handling: pandas exceptions vs swiftpandas exit codes
set -euo pipefail
...
script_banner "10 — Error Handling" "Pandas Python exceptions vs swiftpandas exit codes"
script_meta "Input:" "$DEMO_CSV"
script_meta "Why:"   "Scripts need deterministic exit codes; pandas raises exceptions"

demo_intro <<EOF
What it does:
  Trigger six different failure modes (unknown column, unknown op, bad
  groupby, missing DF handle, missing file, bad drop) and see how each
  side surfaces the error.

Expected outcome: swiftpandas ~3-5× faster, AND meaningfully better
                  shell-script ergonomics.

Why:
  • Time-wise: same import tax for pandas; swiftpandas pays only wire
    round trips.
  • The bigger win is the **error contract**:
      - pandas raises Python exceptions (KeyError, AttributeError, etc.)
        which a shell script can only detect by parsing stderr.
      - swiftpandas returns stable exit codes documented in
        docs/SERVER.md (2 = no server, 3 = ok:false, 4 = transport).
        Scripts can branch deterministically on $?.
  • The wire reply also includes a stable 'error.code' string
    ('no_such_df', 'unknown_column', 'io', etc.) for programmatic
    handling — never a free-form Python traceback.
EOF

PANDAS_T="—"
if pandas_available; then
    pandas_section
    show_code "Python equivalent (raises exceptions)" <<'EOF'
df = pd.read_csv("sales.csv")
df["nonexistent"]              # KeyError
df.explode_nonsense()          # AttributeError
df.agg({"x": "sum"})           # KeyError on missing column
ghost_df.head()                # NameError
pd.read_csv("/missing.csv")    # FileNotFoundError
df.drop(columns=["never"])     # KeyError
EOF
    output_label
    time_start
    $PY <<PYEOF
...
def show(label, fn):
    print(f"--- {label} ---")
    try:
        fn(); print("    (no error raised)")
    except Exception as e:
        line = f"raised {type(e).__name__}: {e}".replace("\n", " | ")
        print(f"    {line[:140]}")

show("1: Unknown column",       lambda: df["nonexistent"])
show("2: Unknown method",       lambda: df.explode_nonsense())
show("3: agg on missing col",   lambda: df.agg({"x": "sum"}))
show("4: Unknown variable",     lambda: ghost_df.head())
show("5: File not found",       lambda: pd.read_csv("/no/such/file.csv"))
show("6: Drop missing column",  lambda: df.drop(columns=["never-existed"]))
PYEOF
...
swiftpandas_section
show_code "swiftpandas commands (return deterministic exit codes)" <<EOF
# Exit codes from docs/SERVER.md:
#   0 = ok
#   2 = no server running
#   3 = daemon returned ok:false  ← most operational errors
#   4 = transport error
swiftpandas pipe --from sales --name err -c "filter(nonexistent > 0)"  # exit 3
swiftpandas pipe --from sales --name err -c "explode(col)"             # exit 3
swiftpandas pipe --from sales --name err -c "agg(sum:revenue)"         # exit 3
swiftpandas pipe --from ghost --name err -c "head(1)"                  # exit 3 (no_such_df)
swiftpandas load /nonexistent/path.csv --name err                      # exit 3 (io)
swiftpandas drop never-existed                                       # exit 3 (no_such_df)
EOF

run_and_show_exit() {
    local label="$1"; shift
    printf "    ${DIM}── %s ──${RESET}\n" "$label"
    "$@" 2>&1 | sed 's/^/      /' || echo "      ${YELLOW}↳ exit $?${RESET}"
}

output_label
time_start
run_and_show_exit "1: Unknown column (filter on nonexistent col)" \
    "$SWIFTPANDAS" pipe --from sales --name err1 -c "filter(nonexistent > 0)"
run_and_show_exit "2: Unknown operation" \
    "$SWIFTPANDAS" pipe --from sales --name err2 -c "explode(col)"
run_and_show_exit "3: agg without groupby" \
    "$SWIFTPANDAS" pipe --from sales --name err3 -c "agg(sum:revenue)"
run_and_show_exit "4: Unknown dataframe (--from ghost)" \
    "$SWIFTPANDAS" pipe --from ghost --name err4 -c "head(1)"
run_and_show_exit "5: File not found (load)" \
    "$SWIFTPANDAS" load /nonexistent/path.csv --name err5
run_and_show_exit "6: Drop a name that isn't bound" \
    "$SWIFTPANDAS" drop never-existed
