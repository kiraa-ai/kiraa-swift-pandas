...
script_banner "01 — Basic Filter" "Keep rows where revenue > 10000"
script_meta "Input:"    "$DEMO_CSV"
script_meta "Pipeline:" 'filter(revenue > 10000)'

demo_intro <<EOF
What it does:
  A one-shot row filter on an 8-row CSV. Both implementations read the
  CSV, keep rows where revenue > 10000, and print the result.

Expected outcome: swiftpandas ~5-10× faster than pandas.

Why:
  • Python has to start the interpreter (~50 ms) and 'import pandas'
    (~600 ms) on every invocation. That ~650 ms of fixed cost dwarfs the
    actual filter work (microseconds for 8 rows).
  • swiftpandas talks to a daemon that's already running with 'sales'
    parsed and in memory. The whole CLI invocation is one Unix-domain
    socket round trip (~5 ms) + the actual filter (microseconds) +
    rendering the CSV reply.
  • For a workload this small, **startup cost is everything**. swiftpandas
    skips it; pandas pays it on every run.
EOF
...
swiftpandas pipe --from sales --name r01 -c "filter(revenue > 10000)"
swiftpandas show r01
swiftpandas server stop                        # (stopped on script exit)
EOF
output_label
time_start
"$SWIFTPANDAS" pipe --from sales --name r01 -c "filter(revenue > 10000)" >/dev/null
"$SWIFTPANDAS" show r01 | sed 's/^/    /'
